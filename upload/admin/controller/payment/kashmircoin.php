...
class ControllerPaymentKashmirCoin extends Controller {
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/'.$this->payment_module_name)) {
        	...
        	if (!$this->request->post['kashmircoin_rpc_username']) {
			$this->error['username'] = $this->language->get('error_username');
		}

	        if (!$this->request->post['kashmircoin_rpc_address']) {
        		$this->error['address'] = $this->language->get('error_address');
	        }

        	if (!$this->request->post['kashmircoin_rpc_password']) {
		        $this->error['password'] = $this->language->get('error_password');
	        }

        	if (!$this->request->post['kashmircoin_rpc_port']) {
		        $this->error['port'] = $this->language->get('error_port');
	        }

        	if (!$this->request->post['kashmircoin_prefix']) {
		        $this->error['prefix'] = $this->language->get('error_prefix');
	        }

        	if (!$this->request->post['kashmircoin_countdown_timer']) {
	...
	public function index() {
		...
		$this->data['text_2'] = $this->language->get('text_2');
        	...
        	$this->data['entry_show_ksc'] = $this->language->get('entry_show_ksc');
	        $this->data['entry_ksc_decimal'] = $this->language->get('entry_ksc_decimal');
		...
		//end language code
        	...
        	$this->data['breadcrumbs'][] = array(
	            ...
	            'href'      => $this->url->link('payment/kashmircoin', 'token=' . $this->session->data['token'], 'SSL'),
	            'separator' => ' :: '
        	);
		...
		$rows = $result->rows;
		$max = $result->num_rows;
		$kashmircoin_total_in_db = 0;
		$kashmircoin_address_in_db = 0;

		for($i = 0;$i < $max;$i++) {
			if($rows[$i]["Field"] == "kashmircoin_total") {
				$kashmircoin_total_in_db = 1;
			}
			if($rows[$i]["Field"] == "kashmircoin_address") {
				$kashmircoin_address_in_db = 1;
			}
		}

		if(!$kashmircoin_total_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD kashmircoin_total DOUBLE AFTER currency_value;");
		}

		if(!$kashmircoin_address_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD kashmircoin_address VARCHAR(34) AFTER kashmircoin_total;");
		}
		...
		if (isset($this->request->post[$this->payment_module_name.'_prefix'])) {
	        ...
	        if (isset($this->request->post[$this->payment_module_name.'_show_ksc'])) {
			$this->data[$this->payment_module_name.'_show_ksc'] = $this->request->post[$this->payment_module_name.'_show_ksc'];
		} else {
			$this->data[$this->payment_module_name.'_show_ksc'] = $this->config->get($this->payment_module_name.'_show_ksc');
		}

	        if (isset($this->request->post[$this->payment_module_name.'_ksc_decimal'])) {
			$this->data[$this->payment_module_name.'_ksc_decimal'] = $this->request->post[$this->payment_module_name.'_ksc_decimal'];
		} else {
			$this->data[$this->payment_module_name.'_ksc_decimal'] = $this->config->get($this->payment_module_name.'_ksc_decimal');
		}
